jQuery(document).ready(function($) {
	...
	// colorpicker field
	$('.bodega-color-picker').each(function(){
		var $this = $(this),
			id = $this.attr('rel');

		$this.farbtastic('#' + id);
	});
	$('.bodega-color-select').click(function(){
		$parent = $(this).parent();
		$(this).siblings('.bodega-color-picker').toggle();
		$('.bodega-color-select', $parent).toggle();
		return false;
	});

	// add more file
	$('.bodega-add-file').click(function(){
	...
	// delete file
	$('.bodega-upload').delegate('.bodega-delete-file', 'click' , function(){
		var $this = $(this),
			$parent = $this.parent(),
			data = $this.attr('rel');
	...
	$('.bodega-images').each(function(){
		...
		$this.sortable({
			...
			update: function (){
				...
				data = order + '|' + $this.siblings('.bodega-images-data').val();
	...
	// thickbox upload
	$('.bodega-upload-button').click(function(){
	...
	// thickbox upload
    window.formfield = '';
    $('.bodega-upload-single-image').live('click', function() {
        window.formfield = $('.bodega-single-image',$(this).parent());
