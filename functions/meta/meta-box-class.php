...
class workz_meta_box {
	...
	function show() {
		...
		wp_nonce_field(basename(__FILE__), 'workz_meta_box_nonce');
	...
	function show_field_text($field, $meta) {
		...
		echo "<input type='text' class='bodega-text' name='{$field['id']}' id='{$field['id']}' value='$meta' size='30' />";
	...
	function save($post_id) {
		...
		|| (!check_admin_referer(basename(__FILE__), 'workz_meta_box_nonce'))		// verify nonce
		...
		foreach ($this->_fields as $field) {
			...
			// validate meta value
			if (class_exists('workz_meta_box_Validate') && method_exists('workz_meta_box_Validate', $field['validate_func'])) {
				$new = call_user_func(array('workz_meta_box_Validate', $field['validate_func']), $new);
			}
